...
func InitBloomFilterTool(n uint, fp float64) *BloomFilterTool {
	return &BloomFilterTool{n, fp}
}
...
// set BloomFilter
func (bft *BloomFilterTool) Update(key string, values []uint64, keyExpire int) (int, error) {
	var cnt int
	if len(values) < 1 {
	...
	json_BloomFilterStorage, err := RedisGetBytes(key)
	if err != nil {
	...
	err = RedisSet(key, []byte(json_BloomFilterStorage), keyExpire)
	if err != nil {
...
func (bft *BloomFilterTool) UpdateWithNotConfirmExist(key string, values []uint64, keyExpire int) (int, error) {
	var cnt int
	if len(values) < 1 {
		return cnt, errors.New("<key, values>, need values")
	}
